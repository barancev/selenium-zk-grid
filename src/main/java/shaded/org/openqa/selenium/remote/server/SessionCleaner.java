...
import shaded.org.openqa.selenium.remote.server.handler.DeleteSession;
import shaded.org.openqa.selenium.remote.server.log.LoggingManager;
import shaded.org.openqa.selenium.remote.server.log.PerSessionLogHandler;
...
class SessionCleaner extends Thread {   // Thread safety reviewed
  ...
  void checkExpiry() {
    for (SessionId sessionId : driverSessions.getSessions()) {
      shaded.org.openqa.selenium.remote.server.Session session = driverSessions.get(sessionId);
      if (session != null) {
